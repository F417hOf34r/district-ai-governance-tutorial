// District Cybersecurity & AI Governance Tutorial, assessment content
...
export const DOMAINS = [
  {
    ...
    optional: false,
    sources: [
      { label: 'State AI guidance tracker (Ballotpedia)', url: 'https://ballotpedia.org/AI_guidance_issued_by_state_departments_of_education' },
    ...
    questions: [
      {
        ...
        levels: [
          ...
          { score: 2, label: 'Mostly in place', description: 'A written AI policy has been adopted by the board and is referenced in district materials, but it has gaps, it may not cover all state-required elements, may not have been widely communicated, or may not have been reviewed since initial adoption.', note: 'A policy with gaps still creates exposure. If the policy doesn’t address IEP restrictions or COPPA consent, it doesn’t protect the district in those areas.', example: 'The board adopted a one-page AI policy in spring 2024. Most teachers are unaware it exists.' },
          { score: 3, label: 'Fully in place', description: 'A comprehensive, board-adopted AI policy exists that covers all state-required elements. It has been publicly posted, communicated to all staff and students, incorporated into relevant handbooks, and reviewed within the past 12 months. A designated leader is responsible for its implementation.', note: 'A fully adopted, communicated AI policy gives the superintendent a documented, defensible position in any board meeting, parent conversation, or regulatory inquiry.', example: 'The board adopted a comprehensive AI policy in January 2026. All staff received training. It is posted on the district website and referenced in the student and staff handbooks.' },
        ],
      },
      {
        id: 'pg-2',
        text: 'Does the AI policy cover all state-required elements?',
        levels: [
          { score: 0, label: 'Not in place', description: 'No policy exists, or the existing policy makes no reference to the elements required by the state education department’s AI framework: ethical use, data privacy, grading implications, acceptable use, and consequences for misuse.', note: 'Missing any of the state-required elements leaves the district exposed in that specific area and out of alignment with state guidance.', example: 'The district has a basic acceptable use policy from 2019 that does not mention AI at all.' },
          { score: 1, label: 'Partially in place', description: 'The policy addresses one or two of the state-required elements, typically acceptable use, but is missing key components such as data privacy provisions, consequences for misuse, or grading and academic integrity guidelines.', note: 'Partial coverage creates uneven protection. A policy that covers acceptable use but not data privacy still leaves the district exposed to FERPA and COPPA risk.', example: 'The policy tells teachers they can use AI for lesson planning but says nothing about what student data they can enter into AI systems.' },
          { score: 2, label: 'Mostly in place', description: 'The policy covers most state-required elements but has identifiable gaps. It may address data privacy but not IDEA/IEP restrictions, or it may cover acceptable use but not define consequences for misuse.', note: 'Even one missing element can create exposure. State frameworks are specific, partial alignment is better than none but does not fully protect the district.', example: 'The policy covers ethical use, acceptable use, and data privacy but doesn’t address how AI intersects with grading or academic integrity.' },
          { score: 3, label: 'Fully in place', description: 'The policy explicitly addresses all state-required elements: ethical use guidelines, data privacy and FERPA/COPPA compliance, grading and academic integrity provisions, acceptable use definitions, consequences for misuse, and the classification system used by the district’s state framework (for example, a stoplight or traffic-light system) categorizing prohibited, permitted, and encouraged AI uses.', note: 'A policy that fully aligns with its state framework positions the district to demonstrate compliance in any state review and respond confidently to any parent or board question about AI use.', example: 'The policy includes an appendix built on the classification system used by the district’s state framework (for example, a stoplight or traffic-light appendix), explicit FERPA/COPPA provisions, academic integrity language, and a consequences matrix, all reviewed by district counsel.' },
        ],
      },
      {
        id: 'pg-3',
      ...
      },
      {
        ...
        levels: [
          { score: 0, label: 'Not in place', description: 'No review process exists. The policy, if one exists, was written once and has not been revisited. No mechanism exists to incorporate regulatory changes, new state guidance, or new AI tools.', note: 'AI regulations and technology are changing faster than any other area of school policy. A policy written in 2024 may already be inadequate in 2026 without any updates.', example: 'The district adopted an AI policy in 2024. No one has looked at it since. The state guidance published the following year was never incorporated.' },
          ...
          { score: 3, label: 'Fully in place', description: 'A formal annual policy review process exists with: a scheduled review date, defined review criteria (checking against current state guidance, federal regulatory changes, new litigation, and new tools), a designated reviewer, a board approval process for any amendments, and documentation of each review.', note: 'A living policy review process means the district is always operating on current regulatory footing.', example: 'The AI policy is reviewed every June by the AI coordinator and legal counsel, compared against any new state guidance published that year, and any amendments are presented to the board at the August meeting.' },
        ],
      },
      {
        id: 'pg-7',
        text: 'Does the district use a traffic light classification system for AI uses?',
        levels: [
    ...
    questions: [
      ...
      },
      {
        ...
        levels: [
          ...
          { score: 2, label: 'Mostly in place', description: 'A formal committee exists with defined membership but it lacks full stakeholder diversity (no parent or student representation), or meets infrequently.', note: 'State guidance specifically calls for diverse representation including parents. A committee without parent voice is out of alignment with state guidance.', example: 'The technology committee reviews AI tools monthly. It includes the technology director, curriculum director, and a teacher representative. No parent or student representation exists.' },
      ...
      {
        id: 'vt-4',
        text: 'Does the district evaluate AI tools against the rubric criteria in its state framework?',
        levels: [
          ...
          { score: 1, label: 'Partially in place', description: 'Some evaluation criteria are applied, typically a basic privacy policy review, but the evaluation does not cover the full state rubric of educational value, data privacy, usability, cost, and age restrictions.', note: 'Partial evaluation creates partial protection. A tool that passes a privacy check may fail on age restrictions.', example: 'The technology director reviews vendor privacy policies before approval. She does not evaluate educational value, vendor reputation, or age restrictions.' },
          { score: 2, label: 'Mostly in place', description: 'A multi-criteria evaluation process exists that covers most of the state rubric dimensions but may not include all of them, or the criteria are not consistently applied.', note: 'Inconsistent application of criteria means some tools receive thorough review while others receive cursory evaluation.', example: 'The review committee uses a 6-criteria checklist. Two of the criteria recommended by the state framework are missing.' },
          { score: 3, label: 'Fully in place', description: 'All AI tool evaluations are conducted against a documented rubric aligned to the state’s criteria, with scores documented for each criterion.', note: 'Systematic rubric-based evaluation produces consistently better tool selections and creates a documented audit trail.', example: 'The AI Review Committee uses a 47-point evaluation rubric aligned to the state’s criteria. Every approved tool has a completed evaluation scorecard on file.' },
        ],
      },
      {
        id: 'vt-5',
...

  {
    ...
    questions: [
      ...
      },
      {
        ...
        levels: [
          { score: 0, label: 'Not in place', description: 'No written incident response plan exists for any data breach scenario.', note: 'State breach-notification laws generally require notice within 30 to 60 days. Without a plan, districts routinely miss deadlines or fail to contain the breach promptly.', example: 'A vendor notifies the district of a breach affecting student data. The superintendent spends the first 3 days trying to figure out who to call.' },
